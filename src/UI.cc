...
void UI_Send(Device::Dji_referee *referee_ui){   
    referee_ui->base_.serial_.write(UI_com, UI_tot);
    UI_tot = 0;
}
...
void UI_Delete( Device::Dji_referee *referee_ui, u8 Del_Operate,u8 Del_Layer)
{
    ...
    framehead.CRC8=referee_ui->base_.getCRC8CheckSum(framepoint,4,0xFF);
    ...
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(framehead),frametail);
    framepoint=(unsigned char *)&datahead;
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(datahead),frametail);
    framepoint=(unsigned char *)&del;
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(del),frametail);  //CRC16校验值计算
...
void Line_Draw(Graph_Data *image,std::string imagename,u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_Width,u32 Start_x,u32 Start_y,u32 End_x,u32 End_y)
{
    int i;
...
void Rectangle_Draw(Graph_Data *image,std::string imagename,u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_Width,u32 Start_x,u32 Start_y,u32 End_x,u32 End_y)
{
    int i;
    for(i=0;i<3&&imagename[i]!='\0';i++)
        image->graphic_name[2-i]=imagename[i];
...
void Circle_Draw(Graph_Data *image,std::string imagename,u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_Width,u32 Start_x,u32 Start_y,u32 Graph_Radius)
{
    int i;
    for(i=0;i<3&&imagename[i]!='\0';i++)
        image->graphic_name[2-i]=imagename[i];
...
void Arc_Draw(Graph_Data *image,std::string imagename,u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_StartAngle,u32 Graph_EndAngle,u32 Graph_Width,u32 Start_x,u32 Start_y,u32 x_Length,u32 y_Length)
{
    int i;
...
void Float_Draw(Float_Data *image,std::string imagename,u32 Graph_Operate,u32 Graph_Layer,u32 Graph_Color,u32 Graph_Size,u32 Graph_Digit,u32 Graph_Width,u32 Start_x,u32 Start_y,float Graph_Float)
{
    int i;
...
void String_Draw(String_Data *image, std::string imagename, u32 Graph_Operate, u32 Graph_Layer, u32 Graph_Color, u32 Graph_Size, u32 Graph_Digit, u32 Graph_Width, u32 Start_x, u32 Start_y, std::string String_Data)
{
    int i;
...
**********************************************************************************************************/
int UI_ReFresh(Device::Dji_referee *referee_ui, int cnt,...)
{
    int i,n;
    ...
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(framehead),frametail);
    framepoint=(unsigned char *)&datahead;
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(datahead),frametail);          //CRC16校验值计算（部分）
    ...
    for(i=0;i<cnt;i++)
    {
        ...
        frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(imageData),frametail);             //CRC16校验
...
Tips：：该函数只能推送1，2，5，7个图形，其他数目协议未涉及
**********************************************************************************************************/
int String_ReFresh(Device::Dji_referee *referee_ui, String_Data string_Data)
{
    int i;
    ...
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(datahead),frametail);
    framepoint=(unsigned char *)&imageData;
    frametail=referee_ui->base_.getCRC16CheckSum(framepoint,sizeof(imageData),frametail);             //CRC16校验   //CRC16校验值计算（部分）
...
void UI_open(Device::Dji_referee *referee_ui, UI_control_t *UI) {
    UI->UI_mode = 1;
    ...
    UI_ReFresh(referee_ui ,2, UI->fric_graph, UI->spin_graph);
    UI_ReFresh(referee_ui, 1, UI->accuracy_graph);
    String_ReFresh(referee_ui, UI->fric_name);
    String_ReFresh(referee_ui, UI->spin_name);
}

void UI_close(Device::Dji_referee *referee_ui, UI_control_t *UI){
    UI->UI_mode = 0;
    UI_Delete(referee_ui, UI_Data_Del_ALL, 9);
}
...
void UI_set_control(Device::Dji_referee *referee_ui, UI_control_t *UI){
    if(UI_control.UI_mode != UI_control.UI_open) {
        if(UI_control.UI_open){
            UI_open(referee_ui, &UI_control);
        }
        else{
            UI_close(referee_ui, &UI_control);
        }
    }
...
void UI_task(Device::Dji_referee *referee_ui){
    UI_init(&UI_control);
    while(1){
        UI_set_mode(&UI_control);
        UI_set_control(referee_ui, &UI_control);
        vTaskDelay(10);
    }
}
...
void draw_crosshair_hero(Device::Dji_referee *referee_ui);
void draw_crosshair_infantry(Device::Dji_referee *referee_ui); 
void update_dynamic_paramater(Device::Dji_referee *referee_ui);
void UI_clear(Device::Dji_referee *referee_ui);
void custom_UI_init(Device::Dji_referee *referee_ui);
...
void UI_init_draw(Device::Dji_referee *referee_ui);
void Read_Robot_ID(Device::Dji_referee *referee_ui);
...
void custom_ui_task(Device::Dji_referee *referee_ui)
{
    custom_UI_init(referee_ui);
    sync_parameter();
    UI_init_draw(referee_ui);
    ...
    while(1)
    {
        ...
        if(count >= RE_INIT_CYCLE)
        {
            Read_Robot_ID(referee_ui);
            osDelay(100);
            if(UI_MODE == UI_HERO) draw_crosshair_hero(referee_ui);
            else if(UI_MODE == UI_INFANTRY) draw_crosshair_infantry(referee_ui);
            osDelay(100);
            UI_init_draw(referee_ui);
            count = 0;
        } 
...
void update_ui_data(Device::Dji_referee *referee_ui){
    UI_Data.distance = ;
    UI_Data.auto_aim_state = ;
    UI_Data.fric_state = ;
    UI_Data.shoot_speed = ;
    UI_Data.spin_state = ;
    UI_Data.Super_cap_percent = ;
}
...
void draw_crosshair_hero(Device::Dji_referee *referee_ui)
{
    ...
    UI_ReFresh(referee_ui, 7, still_cross_line[0], still_cross_line[1], still_cross_line[2],
...
void draw_crosshair_infantry(Device::Dji_referee *referee_ui)
{
    ...
    Line_Draw(&still_cross_line[1], "L2", UI_Graph_ADD, 0, 
        ...
        Crosshair_Data.center[1]-(Crosshair_Data.cross_high/2)+Crosshair_Data.cross_high_offset);
    //相比英雄没有标尺
        UI_ReFresh(referee_ui, 2, still_cross_line[0], still_cross_line[1]);
...
void UI_init_draw(Device::Dji_referee *referee_ui)
{
    ...
    Line_Draw(&cap_percentage, "cap", UI_Graph_ADD, 1, State_Data.cap_bar_color, State_Data.cap_display_with,
                ...
                State_Data.cap_text_pos[1] - State_Data.cap_text_size*4.8);
    UI_ReFresh(referee_ui, 2, shoot_distance_bar, cap_percentage);
    osDelay(100);
    ...
    String_ReFresh(referee_ui, state_text_data);
    osDelay(150);
    
    //瞄准框
    Rectangle_Draw(&auto_aim_range, "aui", UI_Graph_ADD, 0, UI_Color_Cyan, 3,
            700, 300, 1300, 800);
    UI_ReFresh(referee_ui, 1, auto_aim_range);
    osDelay(100);
...
void update_dynamic_paramater(Device::Dji_referee *referee_ui)
{
    ...
    if(UI_MODE == UI_HERO)  UI_ReFresh(referee_ui, 2, shoot_distance_bar, cap_percentage);
    else if(UI_MODE == UI_INFANTRY) UI_ReFresh(referee_ui, 1, cap_percentage);
    osDelay(100);
    String_ReFresh(referee_ui, state_text_data);
    osDelay(100);
    UI_ReFresh(referee_ui, 1, auto_aim_range);
...
void UI_clear(Device::Dji_referee *referee_ui)
{
    UI_Delete(referee_ui, UI_Data_Del_ALL, 0);
    osDelay(100);
}
...
void Read_Robot_ID(Device::Dji_referee *referee_ui)
{
    Robot_ID_Read = referee_ui->base_.get_robot_id();
